...
function toggleMenu() {
    hamburger.classList.toggle('active'); // toggle alterna entre ativo e inativo
...
document.addEventListener('click', (e) => {
    if (!hamburger.contains(e.target) && !navMenu.contains(e.target)) { // !hamburguer.contains verifica se ñ está contido o evento
        hamburger.classList.remove('active'); // remove a classe
        navMenu.classList.remove('active');
    }
});
...
let currentSlide = 0; // valor inicial
...
function showSlide(index) {
    ...
    // faz o botão interagir com slide para saber qual slide está ⬇️
...
function nextSlide() {
    currentSlide = (currentSlide + 1) % totalSlides; // próximo slide para próximo terço do slide
...
function startAutoPlay() {
    autoPlayInterval = setInterval(nextSlide, 6000); // Muda a cada 5 segundos
}
...
carouselContainer.addEventListener('mouseenter', stopAutoPlay); // o mouse fica sobre o carrossel desativa autoplay
carouselContainer.addEventListener('mouseleave', startAutoPlay); // o mouse fora do carrossel ativa o autoplay
...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
    anchor.addEventListener('click', function (e) {
        ...
        if (targetElement) {
            ...
            const navbarHeight = document.getElementById('navbar').offsetHeight; // .offsetTop pega as medidas
